...
describe('addQuestion Integration Tests', () => {
  ...
  describe('addQuestion API Integration', () => {
    it('should successfully add TEXT question to form', async () => {
      ...
      // Simulate the addQuestion function call structure
      const questionItem = {
        question: {
      ...
      await mockFormsAPI.forms.batchUpdate(expectedBatchUpdateCall);

      // Verify the API was called with correct structure
      expect(mockFormsAPI.forms.batchUpdate).toHaveBeenCalledWith(expectedBatchUpdateCall);
      expect(mockFormsAPI.forms.batchUpdate).toHaveBeenCalledTimes(1);

      // Verify the questionItem structure doesn't have required at root level
      const calledWith = mockFormsAPI.forms.batchUpdate.mock.calls[0]![0] as any;
      ...
      expect((actualQuestionItem as any).required).toBeUndefined();
    });
    ...
    it('should successfully add MULTIPLE_CHOICE question to form', async () => {
      ...
      const calledWith = mockFormsAPI.forms.batchUpdate.mock.calls[1]![0] as any;
      const actualQuestionItem = calledWith.requestBody.requests[0].createItem.item.questionItem;
    ...
    it('should successfully add LINEAR_SCALE question to form', async () => {
      ...
      const questionItem = {
        question: {
          ...
          scaleQuestion: {
            low: args.scaleMin,
            high: args.scaleMax,
            lowLabel: args.scaleMinLabel,
            highLabel: args.scaleMaxLabel,
          },
        },
      };
      ...
      const calledWith = mockFormsAPI.forms.batchUpdate.mock.calls[2]![0] as any;
      const actualQuestionItem = calledWith.requestBody.requests[0].createItem.item.questionItem;
    ...
    it('should handle DATE question correctly', async () => {
      ...
      const calledWith = mockFormsAPI.forms.batchUpdate.mock.calls[3]![0] as any;
      const actualQuestionItem = calledWith.requestBody.requests[0].createItem.item.questionItem;
    ...
    it('should handle TIME question correctly', async () => {
      ...
      const calledWith = mockFormsAPI.forms.batchUpdate.mock.calls[4]![0] as any;
      const actualQuestionItem = calledWith.requestBody.requests[0].createItem.item.questionItem;
  ...
  describe('Error Handling', () => {
    it('should reject when Google Forms API returns an error', async () => {
      ...
      await expect(mockFormsAPI.forms.batchUpdate(batchUpdateCall)).rejects.toThrow('API Error: Invalid form ID');
    });
  ...
  describe('Backward Compatibility', () => {
    it('should maintain compatibility with existing form operations', async () => {
      ...
      const getResult = await mockFormsAPI.forms.get({ formId: 'existing-form-id' }) as any;
