...
Google Workspace MCP is remote-only. MCP clients connect to the Cloudflare Workers `/mcp` endpoint by URL.
...
- [Key Rotation](./07-key-rotation.md)

Do not use legacy local stdio, local HTTP, Docker, Redis, or local bootstrap instructions as current setup guidance.
