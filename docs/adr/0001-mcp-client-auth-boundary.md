...
The gdrive MCP server is a Google Workspace MCP server, not an identity provider. MCP client authentication is static bearer auth for this compatibility slice, while Google OAuth remains the upstream authorization mechanism from this server to Google Workspace APIs.

This means Codex can only initiate a login flow if this server advertises a real external or dedicated OAuth authorization server in MCP metadata. This repo must not fake OAuth metadata or implement `/authorize` and `/token` endpoints as part of the Google Workspace server just to make `codex mcp login gdrive` appear supported.
